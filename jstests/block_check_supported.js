...
var doTest = function() {
    ...
    if (isXP) {
        ...
        assert.eq(indexCounters_2.accesses, indexCounters_1.accesses + 1,
                  'Test FAILED: db.serverStatus().indexCounters.accesses' +
                  ' should have had the value ' + (indexCounters_1.accesses + 1) +
                  ': indexCounters: before find(): ' + tojsononeline(indexCounters_1) +
                  ', after find(): ' + tojsononeline(indexCounters_2));
        assert.eq(indexCounters_2.hits, indexCounters_1.hits + 1,
                  'Test FAILED: db.serverStatus().indexCounters.hits' +
                  ' should have had the value ' + (indexCounters_1.hits + 1) +
                  ': indexCounters: before find(): ' + tojsononeline(indexCounters_1) +
                  ', after find(): ' + tojsononeline(indexCounters_2));
    }
    print('Test PASSED!');
