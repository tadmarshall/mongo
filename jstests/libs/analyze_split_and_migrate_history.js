 ...
 * @return              display string (multiline) containing history analysis
 */
var analyze_split_and_migrate_history = function(config, ns, label, key, value, insertTime) {
    ...
    if (!finalChunk) {
        print('FATAL analysis logic error!  Cannot find final chunk containing document!');
        out('FATAL logic error while analyzing Split/Migrate/Insert' +
            ' history for document ' + tojson(label) +
            ' { ' + tojson(key) + ': ' + tojson(value) +
            ' } inserted at ' + tojson(insertTime) +
            '!  Cannot find final chunk containing document!');
        return outputString;
    }
