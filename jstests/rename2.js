...
c = "rename2c";
dbc = db.getSiblingDB("test").getCollection(c);
d = "rename2dename2dename2dename2dename2dename2dename2dename2dename2dename2d";
dbd = db.getSiblingDB("test").getCollection(d);
...
assert.eq( 2 , db.getSiblingDB("test").system.indexes.find( { "ns" : "test." + c } ).count() ,  "Long Rename Init" );
...
assert.eq( 2 , db.getSiblingDB("test").system.indexes.find( { "ns" : "test." + c } ).count() ,  "Long Rename Result 1" );
assert.eq( 0 , db.getSiblingDB("test").system.indexes.find( { "ns" : "test." + d } ).count() ,  "Long Rename Result 2" );
