...
// We create documents a bit over 1 KB each in order to encourage splits (and hence migrates)
// so that we can stress document insertion during migration
//
...
for (var iter = 0; iter < numIterations; ++iter) {
    ...
    for (var i = 0; i < numBatch; ++i) {
        ...
        //print('DEBUG: Saving document i: ' + (numDocs + i) + ' a: ' + randomKey + ' at ' + timeNow.tojson());
        testDB.foo.save( {a: randomKey, i: numDocs + i, t: timeNow, y: str} );
    }
    ...
    // Wait until we have at least one chunk on each shard
    //
